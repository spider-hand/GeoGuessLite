...
defineOptions({ name: 'DailyChallengeGameSummary' })
  ...
  const roundMarkers: Array<GameMapMarker> = [
    {
      ...
      label: t('components.pages.Game.DailyChallengeGameSummary.target'),
      markerType: 'target',
    },
  ]
  if (round.selection) {
...
const formatDistance = (distanceKm: number | null) =>
  ...
  t('components.pages.Game.DailyChallengeGameSummary.noGuess')
</script>

<template>
  <section class="daily-challenge-game-summary">
    <GameMap
      v-if="selectedRound"
      class="daily-challenge-game-summary__map"
      :center="center"
      :is-selectable="false"
      :markers="markers"
      :zoom="zoom"
    ...
    <aside class="daily-challenge-game-summary__panel">
      <header class="daily-challenge-game-summary__header">
        <p>{{ t('components.pages.Game.DailyChallengeGameSummary.title') }}</p>
        <h1>{{ formatNumber(props.totalScore, locale) }}</h1>
        <span>{{ t('components.pages.Game.DailyChallengeGameSummary.totalScore') }}</span>
      </header>

      <ol
        class="daily-challenge-game-summary__rounds"
        :aria-label="t('components.pages.Game.DailyChallengeGameSummary.roundHistory')"
      >
        <li v-for="round in props.rounds" :key="round.roundNumber">
          <button
            class="daily-challenge-game-summary__round"
            :class="{
              'daily-challenge-game-summary__round--selected':
          ...
          >
            <span class="daily-challenge-game-summary__round-label">
              {{
                t('components.pages.Game.DailyChallengeGameSummary.round', {
                  round: round.roundNumber,
                })
              }}
            </span>
            <span class="daily-challenge-game-summary__round-result">
      ...
      <Button variant="secondary" @click="emit('home')">
        {{ t('components.pages.Game.DailyChallengeGameSummary.home') }}
      </Button>
    </aside>
  </section>
</template>
...
.daily-challenge-game-summary__map {
  height: min(620px, calc(100vh - 220px));
  min-height: 480px;
}

.daily-challenge-game-summary__panel {
  display: flex;
...
.daily-challenge-game-summary__header p,
.daily-challenge-game-summary__header span {
  margin: 0;
  color: var(--muted-strong);
  font-size: var(--font-size-body-md);
}

.daily-challenge-game-summary__header h1 {
...
.daily-challenge-game-summary__rounds {
  display: grid;
  margin: 0;
  padding: 0;
  list-style: none;
...
.daily-challenge-game-summary__round {
  display: flex;
  align-items: center;
...
.daily-challenge-game-summary__rounds li:first-child .daily-challenge-game-summary__round {
  border-top: 1px solid var(--hairline);
}

.daily-challenge-game-summary__round:hover,
.daily-challenge-game-summary__round--selected {
  background-color: var(--surface-elevated-dark);
}

.daily-challenge-game-summary__round:focus-visible {
  outline: 2px solid color-mix(in srgb, var(--info-ring) 50%, transparent);
  outline-offset: -2px;
}

.daily-challenge-game-summary__round-label {
  font-weight: var(--font-weight-semibold);
}

.daily-challenge-game-summary__round-result {
  display: flex;
  flex-direction: column;
...
.daily-challenge-game-summary__round-result strong {
  color: var(--on-dark);
  font-family: var(--font-number);
}
